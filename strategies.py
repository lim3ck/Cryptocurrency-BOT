...
class TweezerRSIStrategy(Strategy):
    def __init__(self, client, contract: Contract, exchange: str, timeframe: str, balance_pct: float,
                 take_profit: float,
                 stop_loss: float, other_params: Dict):
        super().__init__(client, contract, exchange, timeframe, balance_pct, take_profit, stop_loss, "Breakout")
        ...
        self._rsi_length = other_params['rsi_length']
        self._low_rsi = other_params['low_rsi']
        self._high_rsi = other_params['high_rsi']
    ...
    def _check_signal(self) -> int:
        ...
        # top
        if self.candles[-2].high > self.candles[-2].close > self.candles[-2].open and self.candles[-1].high > \
                self.candles[-1].open > self.candles[-1].close and rsi > self._high_rsi and self._in_range(
            self.candles[-2].close, self.candles[-1].open):
            return -1
        # bottom
        elif self.candles[-2].low < self.candles[-2].close < self.candles[-2].open and self.candles[-1].low < \
                self.candles[-1].open < self.candles[-1].close and rsi < self._low_rsi and self._in_range(
            self.candles[-2].close, self.candles[-1].open):
            return 1
        else:
            return 0
    ...
    def _in_range(self, a: float, b: float) -> bool:
        if a * 1.005 >= b >= a * 0.995:
            return True
        else:
            return False
